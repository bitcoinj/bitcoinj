...
import org.monacoinj.core.AbstractBlockChain;
import org.monacoinj.core.Address;
import org.monacoinj.core.BlockChain;
import org.monacoinj.core.BloomFilter;
import org.monacoinj.core.Coin;
import org.monacoinj.core.Context;
import org.monacoinj.core.ECKey;
import org.monacoinj.core.FilteredBlock;
import org.monacoinj.core.InsufficientMoneyException;
import org.monacoinj.core.Message;
import org.monacoinj.core.NetworkParameters;
import org.monacoinj.core.Peer;
import org.monacoinj.core.PeerFilterProvider;
import org.monacoinj.core.PeerGroup;
import org.monacoinj.core.Sha256Hash;
import org.monacoinj.core.StoredBlock;
import org.monacoinj.core.Transaction;
import org.monacoinj.core.TransactionBag;
import org.monacoinj.core.TransactionBroadcast;
import org.monacoinj.core.TransactionBroadcaster;
import org.monacoinj.core.TransactionConfidence;
import org.monacoinj.core.TransactionInput;
import org.monacoinj.core.TransactionOutPoint;
import org.monacoinj.core.TransactionOutput;
import org.monacoinj.core.UTXO;
import org.monacoinj.core.UTXOProvider;
import org.monacoinj.core.UTXOProviderException;
import org.monacoinj.core.Utils;
import org.monacoinj.core.VarInt;
import org.monacoinj.core.VerificationException;
import org.monacoinj.core.TransactionConfidence.*;
import org.monacoinj.crypto.*;
import org.monacoinj.script.*;
import org.monacoinj.signers.*;
import org.monacoinj.utils.*;
import org.monacoinj.wallet.Protos.Wallet.*;
import org.monacoinj.wallet.WalletTransaction.*;
...
import org.monacoinj.wallet.listeners.WalletChangeEventListener;
...
import org.monacoinj.wallet.listeners.WalletEventListener;
import org.monacoinj.wallet.listeners.WalletReorganizeEventListener;
import org.slf4j.*;
 ...
 * of the Monacoin protocol.</p>
 *
 * <p>To learn more about this class, read <b><a href="https://monacoinj.github.io/working-with-the-wallet">
 ...
 * other objects, see the <a href="https://monacoinj.github.io/getting-started">Getting started</a> tutorial
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, org.monacoinj.wallet.WalletFiles.Listener)}
...
public class Wallet extends BaseTaggableObject
    ...
    // Dead:     Transactions that we believe will never confirm get moved here, out of pending. Note that Monacoin
     ...
     * {@link org.monacoinj.wallet.DeterministicKeyChain#ACCOUNT_ZERO_PATH 0 hardened path}
     */
     ...
     * {@link org.monacoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS}. The returned key is stable until
    ...
    /**
     * An alias for calling {@link #currentKey(org.monacoinj.wallet.KeyChain.KeyPurpose)} with
     * {@link org.monacoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
    ...
    /**
     * Returns address for a {@link #currentKey(org.monacoinj.wallet.KeyChain.KeyPurpose)}
     */
    ...
    /**
     * An alias for calling {@link #currentAddress(org.monacoinj.wallet.KeyChain.KeyPurpose)} with
     * {@link org.monacoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
     ...
     * {@link org.monacoinj.wallet.DeterministicKeyChain}. When the parameter is
     * {@link org.monacoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} the returned key is suitable for being put
    ...
    /**
     * An alias for calling {@link #freshKey(org.monacoinj.wallet.KeyChain.KeyPurpose)} with
     * {@link org.monacoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
    ...
    /**
     * Returns address for a {@link #freshKey(org.monacoinj.wallet.KeyChain.KeyPurpose)}
     */
    ...
    /**
     * An alias for calling {@link #freshAddress(org.monacoinj.wallet.KeyChain.KeyPurpose)} with
     * {@link org.monacoinj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
     ...
     * key, after loading a wallet produced by previous versions of monacoinj. If the wallet is encrypted the key
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, org.monacoinj.wallet.WalletFiles.Listener)}
    ...
    /** See {@link org.monacoinj.wallet.DeterministicKeyChain#setLookaheadSize(int)} for more info on this. */
    ...
    /** See {@link org.monacoinj.wallet.DeterministicKeyChain#setLookaheadThreshold(int)} for more info on this. */
     ...
     * See {@link org.monacoinj.wallet.DeterministicKeyChain#markKeyAsUsed(DeterministicKey)} for more info on this.
     */
     ...
     * @throws org.monacoinj.core.ECKey.MissingPrivateKeyException if the seed is unavailable (watching wallet)
     */
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(org.monacoinj.crypto.KeyCrypter,
     ...
     * {@link org.monacoinj.crypto.KeyCrypterScrypt}.
     *
    ...
    private void processTxFromBestChain(Transaction tx, boolean forceAddToPool) throws VerificationException {
        ...
        if (isDeadCoinbase) {
            ...
            // we could. Monacoin Core nodes on the network have deleted the dependent transactions from their mempools
        ...
        for (TransactionInput input : tx.getInputs()) {
            ...
            if (result == TransactionInput.ConnectionResult.ALREADY_SPENT) {
                if (fromChain) {
                    ...
                    log.warn("{}: {}", tx.getHash(), Utils.HEX.encode(tx.unsafeMonacoinSerialize()));
                    Transaction other = output.getSpentBy().getParentTransaction();
                    log.warn("{}: {}", other.getHash(), Utils.HEX.encode(other.unsafeMonacoinSerialize()));
                }
     ...
     * of {@link org.monacoinj.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
     ...
     * you can use {@link org.monacoinj.utils.Threading#waitForUserCode()} to block until the future had a
    ...
    /**
     * Returns the amount of monacoin ever received via output. <b>This is not the balance!</b> If an output spends from a
    ...
    /**
     * Returns the amount of monacoin ever sent via output. If an output is sent to our own wallet, because of change or
    ...
    public static class SendResult {
        /** The Monacoin transaction message that moves the money. */
    ...
    public enum MissingSigsMode {
        ...
        USE_DUMMY_SIG,
        /**
         * If signature is missing, {@link org.monacoinj.signers.TransactionSigner.MissingSignatureException}
         * will be thrown for P2SH and {@link org.monacoinj.core.ECKey.MissingPrivateKeyException} for other tx types.
         */
        THROW
    }
     ...
     *
     * @param address The Monacoin address to send the money to.
     ...
     * @throws ExceededMaxTransactionSize if the resultant transaction is too big for Monacoin to process.
    ...
    /**
     * Thrown if the resultant transaction is too big for Monacoin to process. Try breaking up the amounts of value.
     */
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException {
        ...
        try {
            ...
            // Check size.
            final int size = req.tx.unsafeMonacoinSerialize().length;
    ...
    private boolean adjustOutputDownwardsForFee(Transaction tx, CoinSelection coinSelection, Coin feePerKb,
        ...
        final int size = tx.unsafeMonacoinSerialize().length + estimateBytesForSigning(coinSelection);
            ...
            for (Sha256Hash blockHash : oldBlockHashes) {
                for (TxOffsetPair pair : mapBlockTx.get(blockHash)) {
                    ...
                    if (tx.isCoinBase()) {
                        ...
                        // this coinbase tx. Some can just go pending forever, like the Monacoin Core. However we
                        // can do our best.
     ...
     * 
     * <p>See the docs for {@link BloomFilter#BloomFilter(int, double, long, org.monacoinj.core.BloomFilter.BloomUpdate)} for a brief explanation of anonymity when using bloom
     * filters.</p>
     */
    @Override @GuardedBy("keyChainGroupLock")
    public BloomFilter getBloomFilter(int size, double falsePositiveRate, long nTweak) {
        ...
        try {
            ...
            for (TransactionOutPoint point : bloomOutPoints)
                filter.insert(point.unsafeMonacoinSerialize());
            return filter;
        } finally {
            endBloomFilterCalculation();
        }
        ...
        while (true) {
            ...
            for (int i = 0; i < req.tx.getOutputs().size(); i++) {
                TransactionOutput output = new TransactionOutput(params, tx,
                        req.tx.getOutputs().get(i).monacoinSerialize(), 0);
                if (req.recipientsPayFees) {
            ...
            int size = tx.unsafeMonacoinSerialize().length;
    ...
    private void addSuppliedInputs(Transaction tx, List<TransactionInput> originalInputs) {
        for (TransactionInput input : originalInputs)
            tx.addInput(new TransactionInput(params, tx, input.monacoinSerialize()));
    }
     ...
     * sending transactions to the Monacoin network by default. Some sendCoins methods let you specify a broadcaster
     ...
     */
    public void setTransactionBroadcaster(@Nullable org.monacoinj.core.TransactionBroadcaster broadcaster) {
     ...
     * @throws org.monacoinj.wallet.DeterministicUpgradeRequiresPassword if key rotation requires the users password.
     */
    ...
    private Transaction rekeyOneBatch(long timeSecs, @Nullable KeyParameter aesKey, List<Transaction> others, boolean sign) {
        ...
        try {
            ...
            checkState(rekeyTx.unsafeMonacoinSerialize().length < Transaction.MAX_STANDARD_TX_SIZE);
            return rekeyTx;
