...
public class TorPeerMonitor {
    ...
    private TorClient setupTor() {
        torClient = new TorClient();
        torClient.start();
        try {
            CircuitPathChooser pathChooser = CircuitPathChooser.create(torClient.getConfig(), torClient.getDirectory());
            ArrayList<ExitTarget> targets = Lists.newArrayList();
            Set<Router> routers = Sets.newHashSet();

            while (routers.size() < 10) {
                Router router = pathChooser.chooseExitNodeForTargets(targets);
                routers.add(router);
            }
            ...
            for (Router router : routers) {
                try {
                    Circuit circuit = torClient.getCircuitManager().openInternalCircuitTo(Lists.newArrayList(router));
            ...
            for (Circuit circuit : circuits) {
                ...
                cell.putString("seed.bitcoin.sipa.be");
                    ...
                    if (res != null) {
                        while (res.cellBytesRemaining() > 0) {
                            ...
                            if (type == 0 || type >= 0xf0)
                                ...
                                System.out.println(InetAddress.getByAddress(value));
                            }
                        }
                        break;
                    }
    ...
    private void setupNetwork() {
        ...
        ClientConnectionManager connectionManager =
                new BlockingClientManager(new OrchidSocketFactory(torClient));
        peerGroup = new PeerGroup(params, null /* no chain */, connectionManager);
