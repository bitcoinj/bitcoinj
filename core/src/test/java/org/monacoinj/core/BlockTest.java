...
public class BlockTest {
    ...
    public void testWork() throws Exception {
        ...
        // This number is printed by Monacoin Core at startup as the calculated value of chainWork on testnet:
        //
    ...
    public void testHeaderParse() throws Exception {
        ...
        Block reparsed = PARAMS.getDefaultSerializer().makeBlock(header.monacoinSerialize());
        assertEquals(reparsed, header);
    }

    @Test
    public void testMonacoinSerialization() throws Exception {
        ...
        //
        // NB: This tests the monacoin serialization protocol.
        Block block = PARAMS.getDefaultSerializer().makeBlock(blockBytes);
        assertTrue(Arrays.equals(blockBytes, block.monacoinSerialize()));
    }
    ...
    public void testUpdateLength() {
        ...
        assertEquals(block.monacoinSerialize().length, block.length);
        final int origBlockLen = block.length;
        Transaction tx = new Transaction(params);
        // this is broken until the transaction has > 1 input + output (which is required anyway...)
        //assertTrue(tx.length == tx.monacoinSerialize().length && tx.length == 8);
        ...
        assertEquals(tx.unsafeMonacoinSerialize().length, tx.length);
        assertEquals(origTxLength, tx.length);
        block.addTransaction(tx);
        assertEquals(block.unsafeMonacoinSerialize().length, block.length);
        ...
        assertEquals(block.length, block.unsafeMonacoinSerialize().length);
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTxLength - 1);
