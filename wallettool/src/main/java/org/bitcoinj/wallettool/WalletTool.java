...
public class WalletTool implements Callable<Integer> {
    ...
    private void createWallet(NetworkParameters params, File walletFile) throws IOException {
        KeyChainGroupStructure keychainStructure = KeyChainGroupStructure.BIP32;
        ...
        if (seedStr != null) {
            ...
            wallet = Wallet.fromSeed(params, seed, outputScriptType, keychainStructure);
        } else if (watchKeyStr != null) {
            wallet = Wallet.fromWatchingKeyB58(params, watchKeyStr, creationTimeSecs);
        } else {
            wallet = Wallet.createDeterministic(params, outputScriptType, keychainStructure);
        }
        if (password != null)
            wallet.encrypt(password);
