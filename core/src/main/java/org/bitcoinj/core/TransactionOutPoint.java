...
/**
 * A reference to a transaction output. In the near future there will be two implementations of this
 * class: one that simply holds the basic data (`hash` and `index`) and another that holds
 * object references linking it to related `Transaction` or `TransactionOutput` objects.
 */
public interface TransactionOutPoint {
     ...
     */
    class ConnectedTransactionOutPoint implements TransactionOutPoint {
        public static final int BYTES = 36;

        /** Special outpoint that normally marks a coinbase input. It's also used as a test dummy. */
        public static final ConnectedTransactionOutPoint UNCONNECTED =
                new ConnectedTransactionOutPoint(ByteUtils.MAX_UNSIGNED_INTEGER, Sha256Hash.ZERO_HASH);
         ...
         */
        public static ConnectedTransactionOutPoint read(ByteBuffer payload) throws BufferUnderflowException, ProtocolException {
            Sha256Hash hash = Sha256Hash.read(payload);
            long index = ByteUtils.readUint32(payload);
            return new ConnectedTransactionOutPoint(index, hash);
        }

        public ConnectedTransactionOutPoint(long index, Transaction fromTx) {
            super();
        ...
        public ConnectedTransactionOutPoint(long index, Sha256Hash hash) {
            super();
            checkArgument(index >= 0 && index <= ByteUtils.MAX_UNSIGNED_INTEGER, () ->
                    "index out of range: " + index);
            this.index = index;
        ...
        public ConnectedTransactionOutPoint(TransactionOutput connectedOutput) {
        ...
        public boolean equals(Object o) {
            ...
            ConnectedTransactionOutPoint other = (ConnectedTransactionOutPoint) o;
