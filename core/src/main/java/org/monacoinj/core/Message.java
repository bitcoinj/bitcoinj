...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using the Monacoin serialization format.
...
public abstract class Message {
     ...
     * @param protocolVersion Monacoin protocol version.
    ...
    private void selfCheck(byte[] payload, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = monacoinSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Monacoin protocol.
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeMonacoinSerialize()}, which is safe to mutate.
    ...
    public byte[] monacoinSerialize() {
        byte[] bytes = unsafeMonacoinSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the monacoin wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeMonacoinSerialize() {
        ...
        try {
            monacoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (serializer.isParseRetainMode()) {
            ...
            // which means another call to monacoinSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the monacoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    public final void monacoinSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use monacoinSerialize().
     */
    protected void monacoinSerializeToStream(OutputStream stream) throws IOException {
        log.error("Error: {} class has not implemented monacoinSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so MonacoinSerializer can avoid 2 instanceof checks + a casting.
     */
