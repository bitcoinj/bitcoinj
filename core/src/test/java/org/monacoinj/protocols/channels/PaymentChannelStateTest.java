...
import static org.monacoinj.testing.FakeTxBuilder.createFakeTx;
import static org.monacoinj.testing.FakeTxBuilder.makeSolvedTestBlock;
...
public class PaymentChannelStateTest extends TestWithWallet {
    ...
    public void basic() throws Exception {
        ...
        Transaction refund;
        if (useRefunds()) {
            refund = new Transaction(PARAMS, clientV1State().getIncompleteRefundTransaction().monacoinSerialize());
        ...
        Transaction multisigContract = new Transaction(PARAMS, clientState.getContract().monacoinSerialize());
        ...
        // Both client and server are now in the ready state. Simulate a few micropayments of 0.005 monacoins.
        Coin size = HALF_COIN.divide(100);
        Coin totalPayment = Coin.ZERO;
        for (int i = 0; i < 4; i++) {
            byte[] signature = clientState.incrementPaymentBy(size, null).signature.encodeToMonacoin();
        ...
        final Transaction reserializedCloseTx = new Transaction(PARAMS, closeTx.monacoinSerialize());
    ...
    public void setupDoS() throws Exception {
        ...
        if (useRefunds()) {
            // Send the refund tx from client to server and get back the signature.
            Transaction refund = new Transaction(PARAMS, clientV1State().getIncompleteRefundTransaction().monacoinSerialize());
            byte[] refundSig = serverV1State().provideRefundTransaction(refund, myKey.getPubKey());
            assertEquals(PaymentChannelServerState.State.WAITING_FOR_MULTISIG_CONTRACT, serverState.getState());
            // This verifies that the refund can spend the multi-sig output when run.
            clientV1State().provideRefundSignature(refundSig, null);
        ...
        serverState.incrementPayment(CENT.divide(2).subtract(CENT.divide(10)),
                clientState.incrementPaymentBy(CENT.divide(10), null).signature.encodeToMonacoin());
    ...
    public void checkBadData() throws Exception {
        ...
        if (useRefunds()) {
            ...
            byte[] refundTxBytes = clientV1State().getIncompleteRefundTransaction().monacoinSerialize();
        ...
        byte[] multisigContractSerialized = clientState.getContract().monacoinSerialize();
        ...
        byte[] signature2 = clientState.incrementPaymentBy(HALF_COIN.subtract(totalPayment), null).signature.encodeToMonacoin();
    ...
    public void feesTest() throws Exception {
        ...
        // The value is close to clientState.getContractInternal().unsafeMonacoinSerialize().length;
        ...
        byte[] signature = clientState.incrementPaymentBy(Coin.SATOSHI, null).signature.encodeToMonacoin();
        ...
        serverState.incrementPayment(CENT.subtract(totalPayment), payment.signature.encodeToMonacoin());
    ...
    public void serverAddsFeeTest() throws Exception {
        ...
        signature = clientState.incrementPaymentBy(SATOSHI.multiply(20), null).signature.encodeToMonacoin();
    ...
    public void doubleSpendContractTest() throws Exception {
        ...
        doubleSpendContract = new Transaction(PARAMS, doubleSpendContract.monacoinSerialize());
