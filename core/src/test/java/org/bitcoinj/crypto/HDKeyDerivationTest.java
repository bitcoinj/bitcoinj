...
public class HDKeyDerivationTest {
    ...
    public void testGenerate() {
        ...
        assertEquals(HDPath.m(CHILD_NUMBER), keys1.get(0).partialPath());

        List<DeterministicKey> keys2 = HDKeyDerivation.generate(parent, CHILD_NUMBER.num())
                                                        .limit(2)
                                                        .collect(Collectors.toList());
        assertEquals(2, keys2.size());
        assertEquals(HDPath.parsePath("m/1"), keys2.get(0).partialPath());
        assertEquals(HDPath.parsePath("m/2"), keys2.get(1).partialPath());
    }
}
