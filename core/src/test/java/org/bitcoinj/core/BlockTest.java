...
public class BlockTest {
    ...
    public void parseBlockWithHugeDeclaredTransactionsSize() {
        ...
        ByteUtils.writeInt32LE(0, buf); // version
        Sha256Hash.ZERO_HASH.write(buf); // prevBlockHash
        Sha256Hash.ZERO_HASH.write(buf); // merkle root
        ByteUtils.writeInt32LE(0, buf); // time
        ByteUtils.writeInt32LE(Difficulty.EASIEST_DIFFICULTY_TARGET.compact(), buf);
        ByteUtils.writeInt32LE(0, buf); // nonce
        huge.write(buf); // transaction count
