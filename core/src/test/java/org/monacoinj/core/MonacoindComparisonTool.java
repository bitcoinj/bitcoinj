...
/**
 * A tool for comparing the blocks which are accepted/rejected by monacoind/monacoinj
 * It is designed to run as a testnet-in-a-box network between a single monacoind node and monacoinj
 * It is not an automated unit-test because it requires a bit more set-up...read comments below
 */
public class MonacoindComparisonTool {
    private static final Logger log = LoggerFactory.getLogger(MonacoindComparisonTool.class);

    private static NetworkParameters params;
    private static FullPrunedBlockChain chain;
    private static Sha256Hash monacoindChainHead;
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("USAGE: monacoinjBlockStoreLocation runExpensiveTests(1/0) [port=18444]");
        ...
        try {
            H2FullPrunedBlockStore store = new H2FullPrunedBlockStore(params, args.length > 0 ? args[0] : "MonacoindComparisonTool", blockList.maximumReorgBlockCount);
            store.resetStore();
        ...
        final Peer monacoind = new Peer(params, ver, new BlockChain(params, new MemoryBlockStore(params)), new PeerAddress(params, InetAddress.getLocalHost()));
        Preconditions.checkState(monacoind.getVersionMessage().hasBlockChain());
        ...
        monacoind.addConnectedEventListener(Threading.SAME_THREAD, new PeerConnectedEventListener() {
            @Override
            public void onPeerConnected(Peer peer, int peerCount) {
                if (!peer.getPeerVersionMessage().subVer.contains("Satoshi")) {
                    ...
                    System.out.println("************************************************************************************************************************\n" +
                                       ...
                                       "think hard about what you're doing. Seriously, no one has gotten even close to correctly reimplementing Monacoin\n" +
                                       "consensus rules, despite serious investment in trying. It is a huge task and the slightest difference is a huge bug.\n" +
                                       "Instead, go work on making Monacoin Core consensus rules a shared library and use that. Seriously, you wont get it right,\n" +
                                       "and starting with this tester as a way to try to do so will simply end in pain and lost coins.\n" +
                                       "************************************************************************************************************************");
                    System.out.println();
                }
                log.info("monacoind connected");
                // Make sure monacoind has no blocks
                monacoind.setDownloadParameters(0, false);
                monacoind.startBlockChainDownload();
                connectedFuture.set(null);
            }
        });

        monacoind.addDisconnectedEventListener(Threading.SAME_THREAD, new PeerDisconnectedEventListener() {
            @Override
            public void onPeerDisconnected(Peer peer, int peerCount) {
                log.error("monacoind node disconnected!");
                System.exit(1);
            }
        });

        monacoind.addPreMessageReceivedEventListener(Threading.SAME_THREAD, new PreMessageReceivedEventListener() {
            @Override
            public Message onPreMessageReceived(Peer peer, Message m) {
                if (m instanceof HeadersMessage) {
                    if (!((HeadersMessage) m).getBlockHeaders().isEmpty()) {
                        ...
                        log.info("Got header from monacoind " + b.getHashAsString());
                        monacoindChainHead = b.getHash();
                    } else
                        log.info("Got empty header message from monacoind");
                    return null;
                } else if (m instanceof Block) {
                    log.error("monacoind sent us a block it already had, make sure monacoind has no blocks!");
                    System.exit(1);
                    ...
                    for (InventoryItem item : ((GetDataMessage) m).items)
                        if (item.type == InventoryItem.Type.Block) {
                            ...
                            if (currentBlock.block.getHash().equals(item.hash))
                                monacoind.sendMessage(currentBlock.block);
                            else {
                                Block nextBlock = preloadedBlocks.get(item.hash);
                                if (nextBlock != null)
                                    monacoind.sendMessage(nextBlock);
                                else {
                    ...
                    try {
                        ...
                        if (!found)
                            sendHeaders = headers;
                        monacoind.sendMessage(new HeadersMessage(params, sendHeaders));
                        InventoryMessage i = new InventoryMessage(params);
                        for (Block b : sendHeaders)
                            i.addBlock(b);
                        monacoind.sendMessage(i);
                    } catch (Exception e) {
        ...
        monacoindChainHead = params.getGenesisBlock().getHash();
        
        // monacoind MUST be on localhost or we will get banned as a DoSer
        new NioClient(new InetSocketAddress(InetAddress.getByName("127.0.0.1"), args.length > 2 ? Integer.parseInt(args[2]) : params.getPort()), monacoind, 1000);
        ...
        for (Rule rule : blockList.list) {
            if (rule instanceof FullBlockTestGenerator.BlockAndValidity) {
                ...
                // monacoind doesn't request blocks inline so we can't rely on a ping for synchronization
                for (int i = 0; !shouldntRequest && !blocksRequested.contains(nextBlock.getHash()); i++) {
                    int SLEEP_TIME = 1;
                    if (i % 1000/SLEEP_TIME == 1000/SLEEP_TIME - 1)
                        log.error("monacoind still hasn't requested block " + block.ruleName + " with hash " + nextBlock.getHash());
                    Thread.sleep(SLEEP_TIME);
                    if (i > 60000/SLEEP_TIME) {
                        log.error("monacoind failed to request block " + block.ruleName);
                        System.exit(1);
                    }
                }
                if (shouldntRequest) {
                    ...
                    if (blocksRequested.contains(nextBlock.getHash())) {
                        log.error("ERROR: monacoind re-requested block " + block.ruleName + " with hash " + nextBlock.getHash());
                        rulesSinceFirstFail++;
                    }
                }
                // If the block throws, we may want to get monacoind to request the same block again
                if (block.throwsException)
                    blocksRequested.remove(nextBlock.getHash());
                //monacoind.sendMessage(nextBlock);
                locator.clear();
                locator.add(monacoindChainHead);
                monacoind.sendMessage(new GetHeadersMessage(params, locator, hashTo));
                monacoind.ping().get();
                if (!chain.getChainHead().getHeader().getHash().equals(monacoindChainHead)) {
                    rulesSinceFirstFail++;
                    log.error("ERROR: monacoind and monacoinj acceptance differs on block \"" + block.ruleName + "\"");
                }
                if (block.sendOnce)
                ...
                if (mostRecentInv == null && !((MemoryPoolState) rule).mempool.isEmpty()) {
                    log.error("ERROR: monacoind had an empty mempool, but we expected some transactions on rule " + rule.ruleName);
                    rulesSinceFirstFail++;
                } else if (mostRecentInv != null && ((MemoryPoolState) rule).mempool.isEmpty()) {
                    log.error("ERROR: monacoind had a non-empty mempool, but we expected an empty one on rule " + rule.ruleName);
                    rulesSinceFirstFail++;
                    ...
                    if (matches)
                        continue;
                    log.error("monacoind's mempool didn't match what we were expecting on rule " + rule.ruleName);
                    log.info("  monacoind's mempool was: ");
                    for (InventoryItem item : mostRecentInv.items)
                        log.info("    " + item.hash);
                ...
                if (monacoind.getPeerVersionMessage().isGetUTXOsSupported()) {
                    UTXORule r = (UTXORule) rule;
                    UTXOsMessage result = monacoind.getUTXOs(r.query).get();
        ...
        if (unexpectedInvs.get() > 0)
            log.error("ERROR: Got " + unexpectedInvs.get() + " unexpected invs from monacoind");
