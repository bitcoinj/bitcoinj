...
import org.monacoinj.wallet.DeterministicUpgradeRequiredException;
import org.monacoinj.wallet.DeterministicUpgradeRequiresPassword;
...
import org.monacoinj.core.listeners.BlocksDownloadedEventListener;
import org.monacoinj.core.listeners.DownloadProgressTracker;
import org.monacoinj.wallet.MarriedKeyChain;
import org.monacoinj.wallet.Protos;
import org.monacoinj.wallet.SendRequest;
import org.monacoinj.wallet.Wallet;
import org.monacoinj.wallet.WalletExtension;
import org.monacoinj.wallet.WalletProtobufSerializer;
...
import static org.monacoinj.core.Coin.parseCoin;
import static com.google.common.base.Preconditions.checkNotNull;

/**
 * A command line tool for manipulating wallets and working with Monacoin.
 */
public class WalletTool {
    ...
    private static org.monacoin.protocols.payments.Protos.PaymentRequest paymentRequest;
    ...
    public static class Condition {
        ...
        public boolean matchMonacoins(Coin comparison) {
            try {
        ...
        if (location.startsWith("http") || location.startsWith("monacoin")) {
            try {
                ...
                if (location.startsWith("http")) {
                    ...
                    MonacoinURI paymentRequestURI = new MonacoinURI(location);
                    future = PaymentSession.createFromMonacoinUri(paymentRequestURI, verifyPki);
                }
            ...
            try {
                paymentRequest = org.monacoin.protocols.payments.Protos.PaymentRequest.newBuilder().mergeFrom(stream).build();
            } catch(IOException e) {
    ...
    private static void wait(WaitForEnum waitFor) throws BlockStoreException {
        ...
        switch (waitFor) {
            ...
            case BALANCE:
                ...
                if (condition.matchMonacoins(wallet.getBalance(Wallet.BalanceType.ESTIMATED))) {
                    latch.countDown();
                    break;
                }
    ...
    private static void deleteKey() {
        ...
        if (pubkey != null) {
            ...
            try {
                ...
                System.err.println(addr + " does not parse as a Monacoin address of the right network parameters.");
                return;
            }
        }
        if (key == null) {
    ...
    static synchronized void onChange(final CountDownLatch latch) {
        ...
        if (condition.matchMonacoins(balance)) {
