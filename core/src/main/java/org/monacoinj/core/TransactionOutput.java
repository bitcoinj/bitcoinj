...
public class TransactionOutput extends ChildMessage {
    ...
    // These fields are not Monacoin serialized. They are used for tracking purposes in our wallet
    ...
    /**
     * <p>If the output script pays to an address as in <a href="https://monacoin.org/en/developer-guide#term-p2pkh">
     ...
     * the redeem script.</p>
     *
     * <p>P2SH is described by <a href="https://github.com/monacoin/bips/blob/master/bip-0016.mediawiki">BIP 16</a> and
     * <a href="https://monacoin.org/en/developer-guide#p2sh-scripts">documented in the Monacoin Developer Guide</a>.</p>
     *
     * @param networkParameters needed to specify an address
    ...
    /**
     * <p>Gets the minimum value for a txout of this size to be considered non-dust by Monacoin Core
     * (and thus relayed). See: CTxOut::IsDust() in Monacoin Core. The assumption is that any output that would
     * consume more than a third of its value in fees is not something the Monacoin system wants to deal with right now,
     * so we call them "dust outputs" and they're made non standard. The choice of one third is somewhat arbitrary and
     * may change in future.</p>
     *
     * <p>You probably should use {@link org.monacoinj.core.TransactionOutput#getMinNonDustValue()} which uses
     * a safe fee-per-kb by default.</p>
     *
     * @param feePerKb The fee required per kilobyte. Note that this is the same as Monacoin Core's -minrelaytxfee * 3
     */
    public Coin getMinNonDustValue(Coin feePerKb) {
        ...
        // formula is wrong for anything that's not a pay-to-address output, unfortunately, we must follow Monacoin Core's
        // wrongness in order to ensure we're considered standard. A better formula would either estimate the
        // size of data needed to satisfy all different script types, or just hard code 33 below.
        final long size = this.unsafeMonacoinSerialize().length + 148;
