...
import org.monacoin.protocols.payments.Protos.PaymentDetails;
...
import org.monacoinj.wallet.KeyChain.KeyPurpose;
import org.monacoinj.wallet.Wallet.MissingSigsMode;
...
public class SendRequest {
     ...
     * (its value is ignored and set to {@link org.monacoinj.wallet.Wallet#getBalance()} - the fees required
     ...
     * is how mining is incentivized in later years of the Monacoin system when inflation drops. It also provides
     ...
     * when choosing which transactions to add to a block. Note that, to keep this equivalent to Monacoin Core
     * definition, a kilobyte is defined as 1000 bytes, not 1024.</p>
     */
    public Coin feePerKb = Context.get().getFeePerKb();

    /**
     * <p>Requires that there be enough fee for a default Monacoin Core to at least relay the transaction.
    ...
    /**
     * If not null, the {@link org.monacoinj.wallet.CoinSelector} to use instead of the wallets default. Coin selectors are
