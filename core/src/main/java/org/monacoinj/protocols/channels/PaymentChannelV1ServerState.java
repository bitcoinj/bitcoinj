...
public class PaymentChannelV1ServerState extends PaymentChannelServerState {
    ...
    private void signMultisigInput(Transaction tx, Transaction.SigHash hashType,
        ...
        byte[] mySig = signature.encodeToMonacoin();
     ...
     *
     * <p>This will set the state to {@link org.monacoinj.protocols.channels.PaymentChannelServerState.State#CLOSED} if the transaction is successfully broadcast on the network.
     * If we fail to broadcast for some reason, the state is set to {@link org.monacoinj.protocols.channels.PaymentChannelServerState.State#ERROR}.</p>
     *
     * <p>If the current state is before {@link org.monacoinj.protocols.channels.PaymentChannelServerState.State#READY} (ie we have not finished initializing the channel), we
     * simply set the state to {@link org.monacoinj.protocols.channels.PaymentChannelServerState.State#CLOSED} and let the client handle getting its refund transaction confirmed.
     * </p>
     *
