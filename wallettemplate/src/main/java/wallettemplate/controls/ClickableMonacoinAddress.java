...
/**
 * A custom control that implements a clickable, copyable Monacoin address. Clicking it opens a local wallet app. The
 * address looks like a blue hyperlink. Next to it there are two icons, one that copies to the clipboard and another
 * that shows a QRcode.
 */
public class ClickableMonacoinAddress extends AnchorPane {
    ...
    public ClickableMonacoinAddress() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("monacoin_address.fxml"));
            loader.setRoot(this);
    ...
    public String uri() {
        return MonacoinURI.convertToMonacoinURI(address.get(), null, Main.APP_NAME, null);
    }
    ...
    protected void showQRCode(MouseEvent event) {
        ...
        final Main.OverlayUI<ClickableMonacoinAddress> overlay = Main.instance.overlayUI(pane, this);
