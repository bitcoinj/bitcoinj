...
==== *Options*:
* `--output-script-type <TYPE>`: If specified, use that as the upgrade target. Script type for address derivation. Note that upgrade can be made from `Basic` --> `P2PKH` --> `P2WPKH`.
...
[source,bash]
----
wallet-tool upgrade basic.wallet
----
