 ...
 * monacoinj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params, int eventHorizon, Coin feePerKb, boolean ensureMinRequiredFee) {
        log.info("Creating monacoinj {} context.", VersionMessage.MONACOINJ_VERSION);
    ...
    /**
     * Returns the current context that is associated with the <b>calling thread</b>. MonacoinJ is an API that has thread
     ...
     * independently use monacoinj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a monacoinj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using monacoinj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing monacoinj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.monacoinj.utils.ContextPropagatingThreadFactory},
     ...
     * want to create core MonacoinJ objects. Generally, if a class can accept a Context in its constructor and might
     * be used (even indirectly) by a thread, you will want to call this first. Your task may be simplified by using
     * a {@link org.monacoinj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.monacoinj.core.NetworkParameters} specified when this context was (auto) created. The
     * network parameters defines various hard coded constants for a specific instance of a Monacoin network, such as
