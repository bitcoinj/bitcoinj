...
public class TransactionTest {
    ...
    public void testWitnessSignatureP2WPKH() {
        ...
        Transaction txConnected = new Transaction();
        txConnected.addOutput(new TransactionOutput(txConnected, Coin.SATOSHI, new byte[0])); // dummy output to fill index 0
        txIn1.connect(txConnected.addOutput(Coin.COIN.multiply(6), scriptPubKey1)); // so this connected output gets index 1
    ...
    public void parseTransactionWithHugeDeclaredInputsSize() {
        ...
        huge.write(buf); // txin count, txins intentionally missing
    ...
    public void parseTransactionWithHugeDeclaredOutputsSize() {
        ...
        huge.write(buf); // txout count, txouts intentionally missing
        writeInt32LE(0, buf); // lock_time
        checkState(!buf.hasRemaining());
        ((Buffer) buf).rewind();
    ...
    public void parseTransactionWithHugeDeclaredWitnessPushCountSize() {
        ...
        in.write(buf); // txin
        zero.write(buf); // txout count, no txouts
        huge.write(buf); // witness pushes count, pushes intentionally missing
        writeInt32LE(0, buf); // lock_time
        checkState(!buf.hasRemaining());
        ((Buffer) buf).rewind();
