...
import org.monacoinj.wallet.DefaultRiskAnalysis;
...
public class TransactionInput extends ChildMessage {
    ...
    protected void monacoinSerializeToStream(OutputStream stream) throws IOException {
        outpoint.monacoinSerialize(stream);
     ...
     * "from address" is not well defined in Monacoin and you should not assume that senders of a transaction can
     ...
     * in nodes memory pools if the existing version is time locked. See the Contracts page on the Monacoin wiki for
     ...
     * @see TransactionOutPoint#getConnectedRedeemData(org.monacoinj.wallet.KeyBag)
     */
    @Nullable
     ...
     * <a href="https://github.com/monacoin/bips/blob/master/bip-0068.mediawiki">relative lock-time</a> enabled.
     */
    ...
    public TransactionInput duplicateDetached() {
        return new TransactionInput(params, null, monacoinSerialize(), 0);
    }

    /**
     * <p>Returns either RuleViolation.NONE if the input is standard, or which rule makes it non-standard if so.
     * The "IsStandard" rules control whether the default Monacoin Core client blocks relay of a tx / refuses to mine it,
