...
public abstract class DatabaseFullPrunedBlockStore implements FullPrunedBlockStore {
        ...
        for(String sql : getCompatibilitySQL()) {
            ...
            try {
                ...
                throw new BlockStoreException("Database block store is not compatible with the current release.  " +
                        "See monacoinj release notes for further information: " + ex.getMessage());
            } finally {
                if (ps != null && !ps.isClosed()) {
                    ps.close();
                }
    ...
    /**
     * Create a new store for the given {@link org.monacoinj.core.NetworkParameters}.
    ...
    private void createNewStore(NetworkParameters params) throws BlockStoreException {
        try {
            ...
            // The coinbase in the genesis block is not spendable. This is because of how Monacoin Core inits
    ...
    protected void putUpdateStoredBlock(StoredBlock storedBlock, boolean wasUndoable) throws SQLException {
        try {
            ...
            s.setBytes(4, storedBlock.getHeader().cloneAsHeader().unsafeMonacoinSerialize());
     ...
     *
     * <p>The balance {@link org.monacoinj.store.DatabaseFullPrunedBlockStore#getBalanceSelectSQL()} returns
