...
import org.monacoinj.net.StreamConnection;
import org.monacoinj.store.BlockStore;
import org.monacoinj.store.BlockStoreException;
import org.monacoinj.utils.ListenerRegistration;
...
/**
 * <p>A Peer handles the high level communication with a Monacoin node, extending a {@link PeerSocketHandler} which
 * handles low-level message (de)serialization.</p>
 *
 * <p>Note that timeouts are handled by the extended
 * {@link org.monacoinj.net.AbstractTimeoutHandler} and timeout is automatically disabled (using
 * {@link org.monacoinj.net.AbstractTimeoutHandler#setTimeoutEnabled(boolean)}) once the version
...
public class Peer extends PeerSocketHandler {
     ...
     * {@link org.monacoinj.net.NioClientManager#openConnection(java.net.SocketAddress, StreamConnection)}
     * or
     * {@link org.monacoinj.net.NioClient#NioClient(java.net.SocketAddress, StreamConnection, int)}.</p>
     *
    ...
    /**
     * <p>Construct a peer that reads/writes from the given block chain. Transactions stored in a {@link org.monacoinj.core.TxConfidenceTable}
    ...
    private void processVersionMessage(VersionMessage m) throws ProtocolException {
        ...
        // monacoinj is a client mode implementation. That means there's not much point in us talking to other client
    ...
    protected void processAlert(AlertMessage m) {
        try {
            ...
            // BigInteger implementations! See https://github.com/monacoinj/monacoinj/issues/526 for discussion. As
        ...
        try {
            ...
            try {
                if (awaitingFreshFilter != null) {
                    ...
                    // range of blocks and get an inv both times, due to the codepath in Monacoin Core hitting
            ...
            if (blocks.size() > 0 && downloadData && blockChain != null) {
                ...
                for (InventoryItem item : blocks) {
                    if (blockChain.isOrphan(item.hash) && downloadBlockBodies) {
                        ...
                        //
                        // Note that as of June 2012 Monacoin Core won't actually ever interleave blocks pushed as
                        // part of chain download with newly announced blocks, so it should always be taken care of by
                        // the duplicate check in blockChainDownloadLocked(). But Monacoin Core may change in future so
    ...
    private void blockChainDownloadLocked(Sha256Hash toHash) {
        ...
        //
        //   https://en.monacoin.it/wiki/Protocol_specification#getblocks
        //
     ...
     * Once the pong is received the value returned by {@link org.monacoinj.core.Peer#getLastPingTime()} is
     * updated.
    ...
    /**
     * Returns the elapsed time of the last ping/pong cycle. If {@link org.monacoinj.core.Peer#ping()} has never
    ...
    /**
     * Returns a moving average of the last N ping/pong cycles. If {@link org.monacoinj.core.Peer#ping()} has never
     ...
     * after the query returns. <b>Most peers do not support this request. You will need to connect to Monacoin XT
     ...
     * <b>Most peers do not support this request. You will need to connect to Monacoin XT peers if you want
     * this to work.</b>
     *
    ...
    public ListenableFuture<UTXOsMessage> getUTXOs(List<TransactionOutPoint> outPoints, boolean includeMempool) {
        ...
        try {
            ...
            if ((peerVer.localServices & GetUTXOsMessage.SERVICE_FLAGS_REQUIRED) != GetUTXOsMessage.SERVICE_FLAGS_REQUIRED)
                throw new ProtocolException("Peer does not support getutxos protocol flag: find Monacoin XT nodes.");
