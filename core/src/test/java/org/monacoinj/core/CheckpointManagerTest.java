...
public class CheckpointManagerTest {
    ...
    public void shouldThrowNullPointerExceptionWhenCheckpointsNotFound() throws IOException {
        expect(params.getId()).andReturn("org/monacoinj/core/CheckpointManagerTest/notFound");
        replay(params);
    ...
    public void shouldThrowNullPointerExceptionWhenCheckpointsInUnknownFormat() throws IOException {
        expect(params.getId()).andReturn("org/monacoinj/core/CheckpointManagerTest/unsupportedFormat");
        replay(params);
        new CheckpointManager(params, null);
    }
    ...
    public void shouldThrowIllegalStateExceptionWithNoCheckpoints() throws IOException {
        expect(params.getId()).andReturn("org/monacoinj/core/CheckpointManagerTest/noCheckpoints");
        replay(params);
        new CheckpointManager(params, null);
    }
    ...
    public void canReadTextualStream() throws IOException {
        expect(params.getId()).andReturn("org/monacoinj/core/CheckpointManagerTest/validTextualFormat");
        expect(params.getSerializer(false)).andReturn(
                new MonacoinSerializer(params, false));
        expect(params.getProtocolVersionNum(NetworkParameters.ProtocolVersion.CURRENT))
                .andReturn(NetworkParameters.ProtocolVersion.CURRENT.getMonacoinProtocolVersion());
        replay(params);
        new CheckpointManager(params, null);
    }
}
