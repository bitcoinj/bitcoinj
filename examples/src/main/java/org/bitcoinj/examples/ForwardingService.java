...
public class ForwardingService {
    ...
    public static void forward(BitcoinNetwork network, Address address) {
        ...
        CompletableFuture<Transaction> forwardedTxFuture = forwardingService.waitForCoins()
            ...
            .thenCompose(confidence -> {
                ...
                return forwardingService.sendTransaction(sendRequest);
            })
        ...
        forwardedTxFuture.orTimeout(1, TimeUnit.HOURS)
            .thenAccept(
                tx -> System.out.printf("Sent %s onwards and acknowledged by peers! Transaction hash is %s\n", tx.getOutputSum().toFriendlyString(),  tx.getTxId())
            )
            .join();
    }
    ...
    /**
     * Setup a listener to wait for incoming coins
     * @return A future that completes when the wallet detects an incoming (likely unconfirmed) transaction
     */
