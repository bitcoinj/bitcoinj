...
import static org.monacoinj.core.Coin.CENT;
import static org.monacoinj.core.Coin.COIN;
import static org.monacoinj.core.Coin.SATOSHI;
import static org.monacoinj.core.Coin.ZERO;
...
public class MonetaryFormatTest {
    ...
    private static final MonetaryFormat NO_CODE = MonetaryFormat.MONA.noCode();
    ...
    public void standardCodes() throws Exception {
        assertEquals("MONA 0.00", MonetaryFormat.MONA.format(Coin.ZERO).toString());
        assertEquals("mMONA 0.00", MonetaryFormat.MMONA.format(Coin.ZERO).toString());
        assertEquals("µMONA 0", MonetaryFormat.UMONA.format(Coin.ZERO).toString());
    }

    @Test
    public void customCode() throws Exception {
        assertEquals("dMONA 0", MonetaryFormat.UMONA.code(1, "dMONA").shift(1).format(Coin.ZERO).toString());
    }
    ...
    public void noCode() throws Exception {
        assertEquals("0", MonetaryFormat.UMONA.noCode().shift(0).format(Coin.ZERO).toString());
        // Ensure that inserting a code after codes are wiped, works
        assertEquals("dMONA 0", MonetaryFormat.UMONA.noCode().code(1, "dMONA").shift(1).format(Coin.ZERO).toString());
    }

    @Test
    public void codeOrientation() throws Exception {
        assertEquals("MONA 0.00", MonetaryFormat.MONA.prefixCode().format(Coin.ZERO).toString());
        assertEquals("0.00 MONA", MonetaryFormat.MONA.postfixCode().format(Coin.ZERO).toString());
    }

    @Test
    public void codeSeparator() throws Exception {
        assertEquals("MONA@0.00", MonetaryFormat.MONA.codeSeparator('@').format(Coin.ZERO).toString());
    }

    @Test(expected = NumberFormatException.class)
    public void missingCode() throws Exception {
        MonetaryFormat.UMONA.shift(1).format(Coin.ZERO);
    }
    ...
    public void parse() throws Exception {
        ...
        assertEquals(Coin.MILLICOIN, MonetaryFormat.MMONA.parse("1"));
        assertEquals(Coin.MILLICOIN, MonetaryFormat.MMONA.parse("1.0"));
        assertEquals(Coin.MILLICOIN, MonetaryFormat.MMONA.parse("01.0000000000"));
        assertEquals(Coin.MILLICOIN, MonetaryFormat.MMONA.positiveSign('+').parse("+1.0"));
        assertEquals(Coin.MILLICOIN.negate(), MonetaryFormat.MMONA.parse("-1"));
        assertEquals(Coin.MILLICOIN.negate(), MonetaryFormat.MMONA.parse("-1.0"));

        assertEquals(Coin.MICROCOIN, MonetaryFormat.UMONA.parse("1"));
        assertEquals(Coin.MICROCOIN, MonetaryFormat.UMONA.parse("1.0"));
        assertEquals(Coin.MICROCOIN, MonetaryFormat.UMONA.parse("01.0000000000"));
        assertEquals(Coin.MICROCOIN, MonetaryFormat.UMONA.positiveSign('+').parse("+1.0"));
        assertEquals(Coin.MICROCOIN.negate(), MonetaryFormat.UMONA.parse("-1"));
        assertEquals(Coin.MICROCOIN.negate(), MonetaryFormat.UMONA.parse("-1.0"));
