...
/**
 * <p>This command is supported only by <a href="http://github.com/monacoinxt/monacoinxt">Monacoin XT</a> nodes, which
 ...
 * <a href="https://github.com/monacoin/bips/blob/master/bip-0065.mediawiki">BIP 65</a> for more information on this
 * message.</p>
 *
 * <p>Note that this message does not let you query the UTXO set by address, script or any other criteria. The
 * reason is that Monacoin nodes don't calculate the necessary database indexes to answer such queries, to save
...
public class GetUTXOsMessage extends Message {
    ...
    protected void monacoinSerializeToStream(OutputStream stream) throws IOException {
        ...
        for (TransactionOutPoint outPoint : outPoints) {
            outPoint.monacoinSerializeToStream(stream);
        }
    }
