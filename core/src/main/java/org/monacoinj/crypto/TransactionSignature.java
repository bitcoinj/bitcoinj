...
/**
 * A TransactionSignature wraps an {@link org.monacoinj.core.ECKey.ECDSASignature} and adds methods for handling
...
public class TransactionSignature extends ECKey.ECDSASignature {
     ...
     * Because Monacoin Core works via bit testing, we must not lose the exact value when round-tripping
     ...
     * Monacoin Core. DER and the SIGHASH encoding allow for quite some flexibility in how the same structures
     ...
     * and generally violates people's mental model of how Monacoin should work, thus, non-canonical signatures are now
     * not relayed by default.
     */
    public static boolean isEncodingCanonical(byte[] signature) {
        // See Monacoin Core's IsCanonicalSignature, https://monacointalk.org/index.php?topic=8392.msg127623#msg127623
     ...
     * of the type used by Monacoin we have to encode them using DER encoding, which is just a way to pack the two
     * components into a structure, and then we append a byte to the end for the sighash flags.
     */
    public byte[] encodeToMonacoin() {
        try {
     ...
     * @deprecated use {@link #decodeFromMonacoin(byte[], boolean, boolean)} instead}.
     */
    @Deprecated
    public static TransactionSignature decodeFromMonacoin(byte[] bytes,
                                                         boolean requireCanonicalEncoding) throws VerificationException {
        return decodeFromMonacoin(bytes, requireCanonicalEncoding, false);
    }
    ...
    public static TransactionSignature decodeFromMonacoin(byte[] bytes,
        ...
        // Monacoin encoding is DER signature + sighash byte.
        ...
        // In Monacoin, any value of the final byte is valid, but not necessarily canonical. See javadocs for
