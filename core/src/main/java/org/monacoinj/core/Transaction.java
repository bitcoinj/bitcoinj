...
import org.monacoinj.core.TransactionConfidence.ConfidenceType;
...
import org.monacoinj.script.ScriptError;
import org.monacoinj.script.ScriptException;
import org.monacoinj.script.ScriptOpCodes;
import org.monacoinj.signers.TransactionSigner;
import org.monacoinj.utils.ExchangeRate;
import org.monacoinj.wallet.Wallet;
import org.monacoinj.wallet.WalletTransaction.Pool;
...
import static org.monacoinj.core.Utils.*;
 ...
 * the minting of new coins. A Transaction object corresponds to the equivalent in the Monacoin C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of Monacoin and have many powerful features. Read
 * <a href="https://monacoinj.github.io/working-with-transactions">"Working with transactions"</a> in the
 * documentation to learn more about how to use this class.</p>
 *
 * <p>All Monacoin transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage {
    ...
    /**
     * If feePerKb is lower than this, Monacoin Core will treat it as if there were no fee.
     */
    public static final Coin REFERENCE_DEFAULT_MIN_TX_FEE = Coin.valueOf(5000); // 0.05 mMONA

    /**
     * If using this feePerKb, transactions will get confirmed within the next couple of blocks.
     * This should be adjusted from time to time. Last adjustment: February 2017.
     */
    public static final Coin DEFAULT_TX_FEE = Coin.valueOf(100000); // 1 mMONA
     ...
     * @param payload Monacoin protocol formatted byte array containing message content.
    ...
    public Sha256Hash getHash() {
        if (hash == null) {
            hash = Sha256Hash.wrapReversed(Sha256Hash.hashTwice(unsafeMonacoinSerialize()));
        }
        return hash;
    }

    /**
     * Used by MonacoinSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of Monacoin share. In 2011 the value of a coinbase
    ...
    public String toString(@Nullable AbstractBlockChain chain) {
        ...
        if (fee != null) {
            final int size = unsafeMonacoinSerialize().length;
    ...
    public void clearInputs() {
        ...
        this.length = this.unsafeMonacoinSerialize().length;
    }
     ...
     * and requires external synchronization. Please refer to general documentation on Monacoin scripting and contracts
    ...
    /**
     * Same as {@link #addSignedInput(TransactionOutPoint, org.monacoinj.script.Script, ECKey, org.monacoinj.core.Transaction.SigHash, boolean)}
     ...
     * a wrapper around calling {@link Transaction#hashForSignature(int, byte[], org.monacoinj.core.Transaction.SigHash, boolean)}
    ...
    public Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        try {
            ...
            Transaction tx = this.params.getDefaultSerializer().makeTransaction(this.monacoinSerialize());
            ...
            // This step has no purpose beyond being synchronized with Monacoin Core's bugs. OP_CODESEPARATOR
            // is a legacy holdover from a previous, broken design of executing scripts that shipped in Monacoin 0.1.
            ...
            // Set the input to the script of its output. Monacoin Core does this but the step has no obvious purpose as
            ...
            if ((sigHashType & 0x1f) == SigHash.NONE.value) {
                ...
                if (inputIndex >= tx.outputs.size()) {
                    ...
                    // Monacoin implementation. Monacoin Core also contains a bug in handling this case:
                    // any transaction output that is signed in this case will result in both the signed output
                    // and any future outputs to this public key being steal-able by anyone who has
                    // the resulting signature and the public key (both of which are part of the signed tx input).

                    // Monacoin Core's bug is that SignatureHash was supposed to return a hash and on this codepath it
    ...
    @Override
    protected void monacoinSerializeToStream(OutputStream stream) throws IOException {
        uint32ToByteStreamLE(version, stream);
        stream.write(new VarInt(inputs.size()).encode());
        for (TransactionInput in : inputs)
            in.monacoinSerialize(stream);
        stream.write(new VarInt(outputs.size()).encode());
        for (TransactionOutput out : outputs)
            out.monacoinSerialize(stream);
     ...
     * since Monacoin 0.8+ a transaction that did not end its lock period (non final) is considered to be non
    ...
    /**
     * Returns the confidence object for this transaction from the {@link org.monacoinj.core.TxConfidenceTable}
     ...
     * (<a href="https://github.com/monacoin/bips/blob/master/bip-0068.mediawiki">BIP 68</a>) if it is version 2 or
     ...
     * <a href="https://github.com/monacoin/bips/blob/master/bip-0125.mediawiki">full replace-by-fee </a> semantics.
     */
     ...
     * This is useful in certain types of <a href="http://en.monacoin.it/wiki/Contracts">contracts</a>, such as
     * micropayment channels.</p>
     *
     * <p>Note that currently the replacement feature is disabled in Monacoin Core and will need to be
