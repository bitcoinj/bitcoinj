 ...
 *    rethink your life. Seriously, why are you reimplementing Monacoin consensus rules? Instead, go work on making
 *    Monacoin Core consensus rules a shared library and use that. Seriously, you wont get it right, and starting with
 *    this tester as a way to try to do so will simply end in pain and lost coins. SERIOUSLY, JUST STOP!
 *
 * b) Monacoin Core is failing some test in here and you're wondering what test is causing failure. Just stop. There is no
...
public class FullBlockTestGenerator {
    // Used by MonacoindComparisonTool and AbstractFullPrunedBlockChainTest to create test cases
    ...
    public RuleList getBlocksToTest(boolean runBarelyExpensiveTests, boolean runExpensiveTests, File blockStorageFile) throws ScriptException, ProtocolException, IOException {
            ...
            public boolean add(Rule element) {
                if (outStream != null && element instanceof BlockAndValidity) {
                    try {
                        ...
                        byte[] block = ((BlockAndValidity)element).block.monacoinSerialize();
        ...
        // Extend the b24 chain to make sure monacoind isn't accepting b24
        ...
        Block b56;
        try {
            b56 = params.getDefaultSerializer().makeBlock(b57.block.monacoinSerialize());
        ...
        Block b56p2;
        try {
            b56p2 = params.getDefaultSerializer().makeBlock(b57p2.block.monacoinSerialize());
        } catch (ProtocolException e) {
            throw new RuntimeException(e); // Cannot happen.
        }
        ...
        Block b64; NewBlock b64Original;
        {
            ...
            for (Transaction transaction : b64Original.block.getTransactions())
                transaction.monacoinSerialize(stream);
            ...
            checkState(Arrays.equals(stream.toByteArray(), b64.monacoinSerialize()));
        ...
        Block b71 = params.getDefaultSerializer().makeBlock(b72.block.monacoinSerialize());
    ...
    class BlockAndValidity extends Rule {
        ...
        boolean sendOnce; // We can throw away the memory for this block once we send it the first time (if monacoind asks again, its broken)
