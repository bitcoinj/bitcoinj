...
import org.monacoinj.core.PeerAddress;
...
import org.monacoinj.signers.StatelessTransactionSigner;
...
import org.monacoinj.testing.*;
import org.monacoinj.utils.ExchangeRate;
import org.monacoinj.utils.Fiat;
import org.monacoinj.utils.Threading;
import org.monacoinj.wallet.Wallet.BalanceType;
...
import org.monacoinj.wallet.Protos.Wallet.EncryptionType;
...
import static org.monacoinj.testing.FakeTxBuilder.*;
...
public class WalletTest extends TestWithWallet {
    ...
    private void basicSpendingCommon(Wallet wallet, Address toAddress, Address destination, Wallet encryptedWallet) throws Exception {
        ...
        // will attach a small fee. Because the Monacoin protocol makes it difficult to determine the fee of an
    ...
    // identical result to Monacoin Core. However the signatures are not deterministic - signing the same data
    ...
    public void blockChainCatchup() throws Exception {
        ...
        // What we'd really like to do is prove Monacoin Core would accept it .... no such luck unfortunately.
    ...
    public void balances() throws Exception {
        ...
        Transaction send1 = wallet.createSend(OTHER_ADDRESS, valueOf(0, 10));
        // Reserialize.
        Transaction send2 = PARAMS.getDefaultSerializer().makeTransaction(send1.monacoinSerialize());
    ...
    public void isConsistent_duplicates() throws Exception {
        ...
        Transaction txClone = PARAMS.getDefaultSerializer().makeTransaction(tx.monacoinSerialize());
        try {
    ...
    public void doubleSpendUnspendsOtherInputs() throws Exception {
        ...
        send2 = PARAMS.getDefaultSerializer().makeTransaction(send2.monacoinSerialize());
    ...
    public void doubleSpends() throws Exception {
        ...
        byte[] buf = send1.monacoinSerialize();
        buf[43] = 0;  // Break the signature: monacoinj won't check in SPV mode and this is easier than other mutations.
    ...
    public void pending1() throws Exception {
        ...
        final Transaction t1Copy = PARAMS.getDefaultSerializer().makeTransaction(t1.monacoinSerialize());
    ...
    public void spendToSameWallet() throws Exception {
        ...
        assertEquals(ZERO, wallet.getBalance());
        sendMoneyToWallet(AbstractBlockChain.NewBlockType.BEST_CHAIN, outbound1);
        // We should have a balance of 1 MONA after the block is received.
        assertEquals(coin1, wallet.getBalance());
    }
    ...
    public void watchingScriptsBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(1e-12).contains(outPoint.unsafeMonacoinSerialize()));

        sendMoneyToWallet(BlockChain.NewBlockType.BEST_CHAIN, t1);
        assertTrue(wallet.getBloomFilter(1e-12).contains(outPoint.unsafeMonacoinSerialize()));
    }
    ...
    public void marriedKeychainBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(0.001).contains(outPoint.unsafeMonacoinSerialize()));

        sendMoneyToWallet(BlockChain.NewBlockType.BEST_CHAIN, t1);
        assertTrue(wallet.getBloomFilter(0.001).contains(outPoint.unsafeMonacoinSerialize()));
    }

    @Test
    public void autosaveImmediate() throws Exception {
        // Test that the wallet will save itself automatically when it changes.
        File f = File.createTempFile("monacoinj-unit-test", null);
    ...
    public void opReturnTwoOutputsTest() throws Exception {
        // Tests sending transaction where one output transfers MONA, the other one writes OP_RETURN.
        receiveATransaction(wallet, myAddress);
        Transaction tx = new Transaction(PARAMS);
        Coin messagePrice = Coin.ZERO;
        Script script = ScriptBuilder.createOpReturnScript("hello world!".getBytes());
    ...
    public void feeSolverAndCoinSelectionTests2() throws Exception {
        ...
        assertTrue(request15.tx.unsafeMonacoinSerialize().length > 1000);
        ...
        assertTrue(request16.tx.unsafeMonacoinSerialize().length > 1000);
        ...
        int theoreticalMaxLength17 = request17.tx.unsafeMonacoinSerialize().length + myKey.getPubKey().length + 75;
        ...
        Transaction spend17 = request17.tx;
        {
            ...
            final int length = spend17.unsafeMonacoinSerialize().length;
        ...
        int theoreticalMaxLength18 = spend18.unsafeMonacoinSerialize().length + myKey.getPubKey().length + 75;
        ...
        assertTrue(spend18.unsafeMonacoinSerialize().length >= 998);
        assertTrue(spend18.unsafeMonacoinSerialize().length <= 1001);
        ...
        request26.tx.addOutput(CENT.subtract(fee.add(dustMinusOne)),
                OTHER_ADDRESS);
        assertTrue(request26.tx.unsafeMonacoinSerialize().length > 1000);
    ...
    public void completeTxPartiallySignedWithDummySigs() throws Exception {
        byte[] dummySig = TransactionSignature.dummy().encodeToMonacoin();
    ...
    public void totalReceivedSent() throws Exception {
        // Receive 4 MONA in 2 separate transactions
        ...
        // Send 3 MONA in a single transaction
