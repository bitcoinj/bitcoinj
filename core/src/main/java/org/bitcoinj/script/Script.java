...
public class Script {
    ...
    /**
     * Convert a byte array to a "push-data" byte array. That is, create a new array with
     * an {@code OP_PUSHDATAx} opcode followed by the contents of the original array.
     * @param bytes A byte array (length less than 65536)
     * @return push-data opcode pushing the input byte array.
     */
    public static byte[] toPushData(byte[] bytes) {
        if (bytes.length < OP_PUSHDATA1) {
            byte[] result = new byte[bytes.length + 1];
            result[0] = (byte) bytes.length;            // opcode *is* length
