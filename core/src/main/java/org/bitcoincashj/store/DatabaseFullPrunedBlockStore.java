...
public abstract class DatabaseFullPrunedBlockStore implements FullPrunedBlockStore {
        ...
        for(String sql : getCompatibilitySQL()) {
            ...
            try {
                ...
                throw new BlockStoreException("Database block store is not compatible with the current release.  " +
                        "See bitcoincashj release notes for further information: " + ex.getMessage());
            } finally {
                if (ps != null && !ps.isClosed()) {
                    ps.close();
                }
    ...
    /**
     * Create a new store for the given {@link org.bitcoincashj.core.NetworkParameters}.
     ...
     *
     * <p>The balance {@link org.bitcoincashj.store.DatabaseFullPrunedBlockStore#getBalanceSelectSQL()} returns
