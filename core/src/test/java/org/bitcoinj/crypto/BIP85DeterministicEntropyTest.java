 ...
 */
public class BIP85DeterministicEntropyTest {
    ...
    public void testIanColeman() throws Exception {
        ...
        DeterministicKey key = BIP85DeterministicEntropy.getPrivateKey(seedPhrase, "");
        assertEquals("73c5da0a", BIP85DeterministicEntropy.fingerprint(key));
        assertEquals("73c5da0a", BIP85DeterministicEntropy.fingerprint(key.withoutPrivateKey()));

        assertEquals("prosper short ramp prepare exchange stove life snack client enough purpose fold",
                BIP85DeterministicEntropy.deriveBIP85Mnemonic(key, 12, 0));

        assertEquals("prosper short ramp prepare exchange stove life snack client enough purpose fold",
                BIP85DeterministicEntropy.deriveBIP85Mnemonic(key, 0, 12, 0));
        ...
        assertEquals("winter brother stamp provide uniform useful doctor prevent venue upper peasant auto view club next clerk tone fox",
                BIP85DeterministicEntropy.deriveBIP85Mnemonic(key, 18, 0));
        ...
        assertEquals("stick exact spice sock filter ginger museum horse kit multiply manual wear grief demand derive alert quiz fault december lava picture immune decade jaguar",
                BIP85DeterministicEntropy.deriveBIP85Mnemonic(key, 24, 0));
    }
    ...
    public void test12Words() {
        ...
        assertEquals("627ef3a6", BIP85DeterministicEntropy.fingerprint(masterKey));

        String childMnemonic = BIP85DeterministicEntropy.deriveBIP85Mnemonic(masterKey, 12, 0);
    ...
    public void test18Words() {
        ...
        String childMnemonic = BIP85DeterministicEntropy.deriveBIP85Mnemonic(masterKey, 18, 0);
    ...
    public void test24Words() {
        ...
        String childMnemonic = BIP85DeterministicEntropy.deriveBIP85Mnemonic(masterKey, 24, 0);
