...
public class BitcoinURI {
    ...
    public static final String BITCOIN_SCHEME = "zetacoin";
    ...
    public BitcoinURI(@Nullable NetworkParameters params, String input) throws BitcoinURIParseException {
        ...
        if (input.startsWith("zetacoin://")) {
            schemeSpecificPart = input.substring("zetacoin://".length());
        } else if (input.startsWith("zetacoin:")) {
            schemeSpecificPart = input.substring("zetacoin:".length());
        } else {
        ...
        if (addressSplitTokens.length == 0)
            throw new BitcoinURIParseException("No data found after the zetacoin: prefix");
