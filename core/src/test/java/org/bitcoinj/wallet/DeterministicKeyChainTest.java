...
public class DeterministicKeyChainTest {
    ...
    private final boolean useSegwit;
    
    public DeterministicKeyChainTest(boolean useSegwit) {
    ...
    public void setup() {
        ...
        chain = new DeterministicKeyChain(ENTROPY, "", secs, useSegwit);
    ...
    public void deriveAccountOne() throws Exception {
        long secs = 1389353062L;
        DeterministicKeyChain chain1 = new AccountOneChain(ENTROPY, "", secs, useSegwit);
    ...
    static class AccountOneChain extends DeterministicKeyChain {
        public AccountOneChain(byte[] entropy, String s, long secs, boolean useSegwit) {
            super(entropy, s, secs, useSegwit);
        }

        public AccountOneChain(KeyCrypter crypter, DeterministicSeed seed, boolean useSegwit) {
            super(seed, crypter, useSegwit);
        }
    ...
    public void serializeAccountOne() throws Exception {
        ...
        KeyChainFactory factory = new KeyChainFactory() {
            @Override
            public DeterministicKeyChain makeKeyChain(Protos.Key key, Protos.Key firstSubKey, DeterministicSeed seed, KeyCrypter crypter, boolean isMarried, boolean useSegwit) {
                return new AccountOneChain(crypter, seed, useSegwit);
            }

            @Override
            public DeterministicKeyChain makeWatchingKeyChain(Protos.Key key, Protos.Key firstSubKey, DeterministicKey accountKey, boolean isFollowingKey, boolean isMarried, boolean useSegwit) {
    ...
    public void random() {
        ...
        chain = new DeterministicKeyChain(new SecureRandom(), 384, useSegwit);
        chain.setLookaheadSize(10);
    ...
    public void serializeUnencrypted() throws UnreadableWalletException {
        ...
        chain = DeterministicKeyChain.fromProtobuf(keys, null, useSegwit).get(0);
        ...
        assertEquals(key2, chain.findKeyFromPubHash(useSegwit ? key2.getSegwitHash() : key2.getPubKeyHash()));
        assertEquals(key3, chain.findKeyFromPubHash(useSegwit ? key3.getSegwitHash() : key3.getPubKeyHash()));
    ...
    public void encryption() throws UnreadableWalletException {
        ...
        doubled.addAll(serialized);
        final List<DeterministicKeyChain> chains = DeterministicKeyChain.fromProtobuf(doubled, encChain.getKeyCrypter(), useSegwit);
        ...
        DeterministicKey decKey1 = decChain.findKeyFromPubHash(useSegwit ? encKey1.getSegwitHash() : encKey1.getPubKeyHash());
        DeterministicKey decKey2 = decChain.findKeyFromPubHash(useSegwit ? encKey2.getSegwitHash() : encKey2.getPubKeyHash());
    ...
    public void bloom2() throws Exception {
        ...
        chain = DeterministicKeyChain.watch(chain.getWatchingKey().dropPrivateBytes().dropParent(), useSegwit);
        int e = chain.numBloomFilterEntries();
        BloomFilter filter = chain.getFilter(e, 0.001, 1);
        for (DeterministicKey key : keys)
            assertTrue("key " + key, filter.contains(useSegwit ? key.getSegwitHash() : key.getPubKeyHash()));
    }
