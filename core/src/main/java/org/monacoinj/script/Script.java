...
import static org.monacoinj.script.ScriptOpCodes.*;
 ...
 *
 * <p>Monacoin transactions don't specify what they do directly. Instead <a href="https://en.monacoin.it/wiki/Script">a
...
public class Script {
    ...
    public byte[] getProgram() {
        try {
            // Don't round-trip as Monacoin Core doesn't and it would introduce a mismatch.
            if (program != null)
     ...
     * Monacoin Core does something similar.</p>
     */
     ...
     * The concept of a "from address" isn't well defined in Monacoin and you should not assume the sender of a
    ...
    private int findSigInRedeem(byte[] signatureBytes, Sha256Hash hash) {
        checkArgument(chunks.get(0).isOpCode()); // P2SH scriptSig
        int numKeys = Script.decodeFromOpN(chunks.get(chunks.size() - 2).opcode);
        TransactionSignature signature = TransactionSignature.decodeFromMonacoin(signatureBytes, true);
        for (int i = 0 ; i < numKeys ; i++) {
     ...
     * not exist in Monacoin Core. It means blocks containing P2SH transactions that don't match
     * correctly are considered valid, but won't be mined upon, so they'll be rapidly re-orgd out of the chain. This
     * logic is defined by <a href="https://github.com/monacoin/bips/blob/master/bip-0016.mediawiki">BIP 16</a>.</p>
     *
     * <p>monacoinj does not support creation of P2SH transactions today. The goal of P2SH is to allow short addresses
    ...
    private static boolean castToBool(byte[] data) {
        ...
        {
            // "Can be negative zero" - Monacoin Core (see OpenSSL's BN_bn2mpi)
            if (data[i] != 0)
     ...
     * {@link org.monacoinj.core.TransactionInput#verify(org.monacoinj.core.TransactionOutput)} or
     * {@link org.monacoinj.script.Script#correctlySpends(org.monacoinj.core.Transaction, long, Script)}. This method
     * is useful if you need more precise control or access to the final state of the stack. This interface is very
     * likely to change in future.
     *
     * @deprecated Use {@link #executeScript(org.monacoinj.core.Transaction, long, org.monacoinj.script.Script, java.util.LinkedList, java.util.Set)}
     * instead.
     */
    @Deprecated
    ...
    // This is more or less a direct translation of the code in Monacoin Core
    ...
    private static void executeCheckSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        try {
            TransactionSignature sig  = TransactionSignature.decodeFromMonacoin(sigBytes, requireCanonical,
                verifyFlags.contains(VerifyFlag.LOW_S));
    ...
    private static int executeMultiSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        while (sigs.size() > 0) {
            ...
            try {
                TransactionSignature sig = TransactionSignature.decodeFromMonacoin(sigs.getFirst(), requireCanonical);
                Sha256Hash hash = txContainingThis.hashForSignature(index, connectedScript, (byte) sig.sighashFlags);
        ...
        // We uselessly remove a stack object to emulate a Monacoin Core bug.
     ...
     * @deprecated Use {@link #correctlySpends(org.monacoinj.core.Transaction, long, org.monacoinj.script.Script, java.util.Set)}
    ...
    public void correctlySpends(Transaction txContainingThis, long scriptSigIndex, Script scriptPubKey,
        ...
        try {
            txContainingThis = txContainingThis.getParams().getDefaultSerializer().makeTransaction(txContainingThis.monacoinSerialize());
    ...
    /**
     * Get the {@link org.monacoinj.script.Script.ScriptType}.
