...
public class StoredPaymentChannelServerStates implements WalletExtension {
     ...
     * is measured in terms of our local clock, and the refund transaction's lock time is measured in terms of Monacoin
     ...
     * This setter will let you delay the setting of the broadcaster until the Monacoin network is ready.
     *
        ...
        try {
            ...
            for (StoredServerChannel channel : mapChannels.values()) {
                ...
                ServerState.StoredServerPaymentChannel.Builder channelBuilder = ServerState.StoredServerPaymentChannel.newBuilder()
                        ...
                        .setContractTransaction(ByteString.copyFrom(channel.contract.unsafeMonacoinSerialize()))
                        .setMyKey(ByteString.copyFrom(channel.myKey.getPrivKeyBytes()));
                if (channel.majorVersion == 1) {
                    channelBuilder.setClientOutput(ByteString.copyFrom(channel.clientOutput.unsafeMonacoinSerialize()));
                } else {
